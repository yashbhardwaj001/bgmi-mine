...
export default function SlotCounter() {
  ...
  const [slotsFilled, setSlotsFilled] = useState(87);
    ...
    <div className="bg-bg-card rounded-2xl p-8 border border-white/5 shadow-2xl relative overflow-hidden">
      ...
      <div className="space-y-6">
        ...
        <div className="h-2 bg-bg-dark rounded-full overflow-hidden border border-white/5">
          <div
            className={`h-full transition-all duration-1000 ease-out ${isFull ? 'bg-red-700' : 'bg-primary'}`}
        ...
        <div className="flex justify-between items-center pt-2">
          <div className="flex items-center gap-2 text-sm">
            {isFull ? (
              <span className="flex items-center text-red-500 font-bold bg-red-500/10 px-4 py-1.5 rounded-full text-xs uppercase tracking-wide">
                <AlertCircle className="w-4 h-4 mr-2" /> Slot Full
              </span>
            ) : (
              <span className="flex items-center text-primary font-semibold bg-primary/10 px-4 py-1.5 rounded-full text-xs uppercase tracking-wide">
          ...
          <button
            className={`px-8 py-3 rounded-lg font-bold text-sm uppercase tracking-wider transition-all duration-200 ${isFull
                ? 'bg-gray-800 text-gray-500 cursor-not-allowed'
                : 'bg-primary text-bg-dark hover:brightness-110 active:scale-95'
              }`}
            disabled={isFull}
            onClick={() => {
              if (!isFull) window.location.href = '/login.html';
            }}
          >
